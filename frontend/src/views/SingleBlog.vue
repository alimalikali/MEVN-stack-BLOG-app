<template>
  <div v-if="loading" class="flex flex-col items-center space-y-4">
    <div
      class="animate-spin rounded-full h-32 w-32 border-t-4 border-blue-500"
    ></div>
  ...
  <section class="w-full md:w-[70%] mx-auto flex flex-col gap-12" v-else>
    <div class="py-10 relative px- h-[50vh]">
      <img :src="post.post.image.image_url" class="w-full h-full object-cover" />
      <div
        class="absolute top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-[50%] flex-col"
      >
        <h1 class="text-5xl md:9xl text-white text-start">
          {{ post.post.title }}
        </h1>
...
const fetchData = async (id) => {
    try {
        const response = await axios.get(`http://localhost:2000/api/v1/post/${id}`);
